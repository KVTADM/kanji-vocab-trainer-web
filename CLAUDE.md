...
- Le déploiement doit être lancé depuis le vrai Mac de Paul : `api.netlify.com` est injoignable depuis le bac à sable, et le CLI n'y est pas authentifié. Coordination via `05 - A faire sur Mac.md` / `03 - Journal.md` dans `Desktop/KVT/00 - Notes projet/`.
- `supabase.co` est également injoignable depuis le bac à sable : `outils/generer-pages-deck.js` ne peut donc pas y être testé en conditions réelles, seuls ses gabarits le sont (`tests/deck.test.js`). Passer par le connecteur MCP Supabase pour lire ou modifier la base.
