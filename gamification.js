// ============================================================
// Gamification — niveaux, XP, pièces, série quotidienne, boutique.
//
// Ajouté le 24/08/2026 à la demande de Paul : rendre l'usage plus motivant,
// façon jeu, en plus (pas à la place) du classement existant. Principe repris
// de `01 - Décisions techniques.md` / vision produit du 25/07 : tout ce qui
...
// XP/pièces) et donne accès à quelques objets de boutique en plus — jamais
// un raccourci qui remplace la pratique.
//
// Numéros de barème (XP par niveau, prix boutique, seuils) : première passe,
...
// Boost Pro : multiplie XP et pièces gagnées. Le Pro ne touche jamais à
...
// Une pièce par mot répondu correctement — seuil identique à MISS_THRESHOLD
// (app.js) sur l'échelle 0-10 de result.points : sous 6 (= 60%), le mot est
// déjà considéré "raté" ailleurs dans l'app, il ne rapporte rien ici non plus.
const GAMIF_SEUIL_PIECE = 6;

// Bonus de fin de session, seulement si le score dépasse 80%.
const GAMIF_SEUIL_BONUS_SESSION = 80;
const GAMIF_BONUS_SESSION = 10;

// Catalogue boutique v1 : des titres cosmétiques affichés à côté du niveau.
// Aucun ne touche à l'apprentissage (contenu, stats, classement) — décoratif
// uniquement, comme le reste de ce qui se paie dans KVT.
const GAMIF_BOUTIQUE = [
  { id: 'titre-motive', nom: 'Motivé·e', emoji: '🌱', prix: 20, pro: false },
  { id: 'titre-serieux', nom: 'Sérieux·se', emoji: '📘', prix: 60, pro: false },
  { id: 'titre-chasseur', nom: 'Chasseur de kanji', emoji: '🎯', prix: 150, pro: false },
  { id: 'titre-nocturne', nom: 'Réviseur·se nocturne', emoji: '🌙', prix: 250, pro: false },
  { id: 'titre-dojo', nom: 'Légende du dojo', emoji: '🥋', prix: 500, pro: false },
  { id: 'titre-sensei', nom: 'Sensei', emoji: '⛩️', prix: 400, pro: true },
  { id: 'titre-dragon', nom: 'Dragon de jade', emoji: '🐉', prix: 800, pro: true }
];
...
function gagnerPieces(points) {
  ...
  const g = assurerGamification();
  const gain = Math.round(1 * (gamifEstPro() ? GAMIF_BOOST_PRO : 1));
  g.pieces += gain;
  return gain;
}
...
function renderBoutique() {
  ...
  container.innerHTML = `
    ...
    <div class="card gamif-solde">
      ...
      <span style="color:var(--muted); font-size:13px;">Gagnées en révisant — une pièce par mot correct, un bonus si tu finis une session à 80% ou plus. Purement décoratif, comme le reste de ce qui se paie ici : ça ne donne aucun avantage sur le classement.</span>
    </div>
    <div class="boutique-grid">
      ${GAMIF_BOUTIQUE.map(o => {
        const possede = g.inventaire.includes(o.id);
        const bloque = o.pro && !pro;
        ...
        return `
          <div class="card boutique-item ${possede ? 'boutique-item--possede' : ''}">
            ...
            ${possede
              ? `<button class="secondary" data-equiper="${o.id}" ${actif ? 'disabled' : ''}>${actif ? 'Équipé' : 'Équiper'}</button>`
              : bloque
                ? `<button class="secondary" disabled>Réservé Pro</button>`
                : `<button class="primary" data-acheter="${o.id}" ${g.pieces < o.prix ? 'disabled' : ''}>${o.prix} 🪙</button>`
            }
          </div>`;
      }).join('')}
    </div>
